       ...
       for (j=i; j<m; j++)
          if ((k = fwdid[i*m+j]) != ~0) {
             ...
             if (t < m-1) {// see if block k+1 descends from pat[t+1..m-1]
                ...
                node = fwd[(t+1)*m+m-1];
                if ((node == NULLT) || (bwd[f-1]==NULLT)) continue; // rest does not exist 
    ...
    *numocc = Count;
    ...
    for (i=Count-1; i; i--) { 
       ...
       if ((id+1) > nOffset) occaux[i] = Tlength - OffsetArray[i];
          ...
          p_aux3 = (id>>bitsW)*nbits_SB;
          p_aux4 = id*nbits_Offs;
          OccArray[i] = (bitget_aux3() + bitget_aux4()) - OffsetArray[i];
       }
    }
    id = OccArray[0];
    if ((id+1) > nOffset) occaux[0] = Tlength - OffsetArray[0];
       ...
       p_aux3 = (id>>bitsW)*nbits_SB;
       p_aux4 = id*nbits_Offs;
       OccArray[0] = (bitget_aux3() + bitget_aux4()) - OffsetArray[0];
    }   
...
void extract_text(void *index, ulong from, ulong to, byte *snippet, 
                  ulong *snippet_length)
 {
    ...
    *snippet_length = to-from+1;
    ...
    p = NODE(I.fwdtrie, I.bwdtrie, idto);
    while (p&&(posaux > to)) {
    ...
    for (idaux = idto; idaux != idfrom;) {
       ...
       p = NODE(I.fwdtrie, I.bwdtrie, --idaux);
    }
