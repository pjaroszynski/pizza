...
void saveBitmap(FILE *f, bitmap B)
 {
    ...
    fwrite(B->sdata, sizeof(uint), ns, f);
    fwrite(B->bdata, sizeof(byte), ns*nb, f);
    if (B->sdata_0 != NULL) {
       indicator = 1;
       fwrite(&indicator, sizeof(uint), 1, f);
       fwrite(B->sdata_0, sizeof(uint), ns, f);
       fwrite(B->bdata_0, sizeof(byte), ns*nb, f);  
    }
    else {
       indicator = 0;
       fwrite(&indicator, sizeof(uint), 1, f);
...
bitmap loadBitmap(FILE *f, uint n, uint *data)
 {
    ...
    fread(B->sdata, sizeof(uint), ns, f);
    fread(B->bdata, sizeof(byte), ns*nb, f);
    fread(&indicator, sizeof(uint), 1, f);
    if (indicator) {
       B->bdata_0 = malloc (ns*nb*sizeof(byte));
       B->sdata_0 = malloc (ns*sizeof(uint));
       fread(B->sdata_0, sizeof(uint), ns, f);
       fread(B->bdata_0, sizeof(byte), ns*nb, f);         
    }
    else {
       B->sdata_0 = NULL;
       B->bdata_0 = NULL;
