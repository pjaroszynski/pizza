...
class TFMindex {
  private:
    ...
    int LF_1(ulong x, ulong *pos, uchar *c) {
      ulong l=0, r=size_uchar, pos_rel;
      ulong mid=(l+r)/2;
      ulong cmid = C[mid];
      while (l<=r) {
        if (cmid<x)
      ...
      assert(x>C[mid]);
      assert(x<=C[mid+1]);
      ////printf("%lu, %lu, %lu, %lu, %d\n",C[mid],x,C[mid+1],x-C[mid],(long) x- (long) C[mid+1]);fflush(stdout);
      *c = mid;
      pos_rel = x-C[mid];
      alphabetrank->select(mid,pos_rel,pos); // position pos goes from 1 to n
      return 0;
    }
  ...
  */
      ...
      //SetField(sampledpositions,1,this->n,1);
      this->sampled = new BitRankW32Int(sampledpositions,this->n,true,factor);
      
 /* test select 0 y 1 */
//ulong Acumulado=1;
//for (i=1;i<=this->n+10;i++) {
  //printf("%lu, %lu, %lu, %lu%lu\n", this->n, i, sampled->select(i),bitget(sampledpositions,sampled->select(i)),bitget(sampledpositions,sampled->select(i)+1));
  //printf("%lu, %lu, %lu, %lu%lu\n", this->n, i, sampled->select0(i),bitget(sampledpositions,sampled->select0(i)),bitget(sampledpositions,sampled->select0(i)+1));
  //fflush(stdout);
  //if (this->n != sampled->select0(i)) Acumulado++;
//}
/* end test */
...
/*
       ulong posicion;
       ulong posiciones[256];
       for (i=0;i<256;++i) 
         posiciones[i]=0;
       for (i=0;i<n;++i) {
         posiciones[bwt[i]]++;
         alphabetrank->select(bwt[i],posiciones[bwt[i]],&posicion);
         printf("%d, %lu, %lu, %lu\n", bwt[i],posicion-1,i, posicion-1-i);
         if ((posicion-1-i) != 0) printf("Error!!!!!");
       }
*/
/*^fin test para select */          
       delete [] bwt;
       *error=0;
    };
...
for (ulong hh=1; hh <= 1000000; hh*=10)
 ...
 return 0;
    ...
    int display2(uchar *pattern, ulong m, ulong numc, ulong *numocc, uchar **snippet_text, ulong **snippet_lengths) {
       // use the FM-search replacing function Occ(c,1,i) with alphabetrank->rank(c,i)
       uchar *pat=pattern;
       if (map0 != 0 )
         pat = pattern0(pattern,m);
       ...
       if (sp<=ep) {
          ...
          *snippet_text = (uchar *) malloc((*numocc)*(m+2*numc)*sizeof(uchar));
          ...
          while (i<=ep) {
             ...
             j=i+1;
             for (dist=numc;dist<2*numc+m;dist++) {
               LF_1(j,&rank_tmp,&c);
               if (c == 0) break;
               j=rank_tmp;
               if (c == map0)
                 text_aux[dist-skip]='\0';
               else
                 text_aux[dist-skip]=c;
             }
             (*snippet_lengths)[locate]=m+2*numc-skip-(2*numc+m-dist);
             text_aux+=m+2*numc;
             locate ++ ;
             i++;
          }
    ...
    int display_lines(uchar *pattern, ulong m, ulong *numocc, uchar **snippet_text, ulong **snippet_lengths, ulong lb, ulong la) {
       // use the FM-search replacing function Occ(c,1,i) with alphabetrank->rank(c,i)
       ulong numc = (la +lb+1)*70;
       uchar *pat=pattern;
       if (map0 != 0 )
         pat = pattern0(pattern,m);
       uchar c = pat[m-1]; ulong i=m-1;
...
int display(void *index, uchar *pattern, ulong length, ulong numc, ulong *numocc, uchar **snippet_text, ulong **snippet_lengths) {
  ...
  //free(occ);
  int aux = _index->display2(pattern, length, numc, numocc, snippet_text, snippet_lengths);
  return aux;
}
int display_lines (void *index, uchar *pattern, ulong length, ulong *numocc, uchar **snippet_text, ulong **snippet_lengths, ulong lb, ulong la) {
  TFMindex *_index=(TFMindex *) index;
  //ulong *occ;
  //int aux = _index->display(pattern, length, numc, numocc, snippet_text, snippet_lengths, &occ);
  //free(occ);
  int aux = _index->display_lines(pattern, length, numocc, snippet_text, snippet_lengths, lb , la);
  return aux;
}
