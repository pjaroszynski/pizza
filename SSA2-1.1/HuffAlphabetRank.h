...
private:
   ...
   bool select_local(ulong code, ulong i, ulong *pos, THuffAlphabetRank *temp);
public:
   ...
   bool select(uchar c, ulong i, ulong *pos); // returns the position (in pos) of the i-th characters c, return false if such character doesn't exist
