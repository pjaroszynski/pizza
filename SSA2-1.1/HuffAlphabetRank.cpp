...
#include "HuffAlphabetRank.h"
   ...
   // returns the position (in pos) of the i-th characters c, return false if such character doesn't exist
   bool THuffAlphabetRank::select(uchar c, ulong i, ulong *pos) {
      THuffAlphabetRank *temp=this;
      ...
      if (!temp->leaf) {
         if ((code & 1u) == 0) {
            if (!temp->left->leaf) {
              if (!select_local( code >> 1 , i, &pos_aux, temp->left)) return false;
              (*pos) = temp->bitrank->select0(pos_aux)+1;
              return true;
            } else {
              (*pos) = temp->bitrank->select0(i)+1;
              if ((*pos) <= temp->bitrank->lenght_in_bits()) return true; else return false;
            }
         } else {
            if (!temp->right->leaf) {
              if (!select_local( code >> 1 , i, &pos_aux, temp->right)) return false;
              (*pos) = temp->bitrank->select1(pos_aux)+1;
              return true;
            } else {
              (*pos) = temp->bitrank->select1(i)+1;
              if ((*pos) <= temp->bitrank->lenght_in_bits()) return true; else return false;
            }
         } 
      }
   ...
   bool THuffAlphabetRank::select_local(ulong code, ulong i, ulong *pos, THuffAlphabetRank *temp) {
      ulong pos_aux;
      if (!temp->leaf) {
         if ((code & 1u) == 0) {
            if (!temp->left->leaf) {
