...
int csa_save(CSA *SA, FILE *fp){
	fwrite(&(SA->m), sizeof(int),1,fp);
	fwrite(&(SA->two), sizeof(int),1,fp);
	fwrite(&(SA->two2), sizeof(int),1,fp);
	fwrite(&(SA->l), sizeof(int),1,fp);
	fwrite(&(SA->n), sizeof(int),1,fp);
	fwrite(SA->K, sizeof(int), (SIGMA+2),fp);
	fwrite(SA->C, sizeof(int), (SIGMA+1),fp);
	fwrite(SA->C2, sizeof(int), (SIGMA+1),fp);
#ifdef USE_MMAP
	fwrite(&(SA->p_size), sizeof(int),1,fp);
	fwrite(SA->B, sizeof(unsigned short), SA->p_size,fp);
	fwrite(&(SA->i_size), sizeof(int),1,fp);
	fwrite(&(SA->r_size), sizeof(int),1,fp);
	fwrite(SA->R, sizeof(int), SA->r_size,fp);
	fwrite(&(SA->sa_size), sizeof(int),1,fp);
	fwrite(SA->SA, sizeof(int), SA->sa_size,fp);
	fwrite(&(SA->isa_size), sizeof(int),1,fp);
	fwrite(SA->ISA, sizeof(int), SA->isa_size,fp);
#endif
	return 0;
}

CSA *csa_load(FILE *fp){
	CSA *SA;
	int e; // use if you want to be sure of what is load
	SA = (CSA *) malloc(sizeof(CSA));
	initranktables();
	mkdecodetable();
	e=fread(&SA->m, sizeof(int),1,fp);
	e=fread(&SA->two, sizeof(int),1,fp);
	e=fread(&SA->two2, sizeof(int),1,fp);
	e=fread(&SA->l, sizeof(int),1,fp);
	e=fread(&SA->n, sizeof(int),1,fp);
	e=fread(SA->K, sizeof(int), (SIGMA+2),fp);
	e=fread(SA->C, sizeof(int), (SIGMA+1),fp);
	e=fread(SA->C2, sizeof(int), (SIGMA+1),fp);
#ifdef USE_MMAP
	e=fread(&(SA->p_size), sizeof(int),1,fp);
	SA->B = new unsigned short[SA->p_size];
	e=fread(SA->B, sizeof(unsigned short), SA->p_size,fp);
	e=fread(&(SA->i_size), sizeof(int),1,fp);

	e=fread(&(SA->r_size), sizeof(int),1,fp);
	SA->R = new int[SA->r_size];
	e=fread(SA->R, sizeof(int), SA->r_size,fp);
			  
	e=fread(&(SA->sa_size), sizeof(int),1,fp);
	SA->SA = new int[SA->sa_size];
	e=fread(SA->SA, sizeof(int), SA->sa_size,fp);
					  
	e=fread(&(SA->isa_size), sizeof(int),1,fp);
	SA->ISA = new int[SA->isa_size];
	e=fread(SA->ISA, sizeof(int), SA->isa_size,fp);
#endif
	return SA;
}
