...
static_selfindex_lz77* static_selfindex_lz77::build(char* filename, char* filenameidx, unsigned char binaryRev, unsigned char binarySst, unsigned char store_sst_ids){
    ...
    //std::cout<<"Building the index to file "<<(sfile+".lzindex")<<std::endl;
    //FILE* lz77index_fp = fopen((sfile+std::string(".lzindex")).c_str(),"w");
    ...
    DeltaCodes* phrases = new DeltaCodes(length,parsing_len,16);//sampling=8
    //delete [] length;
    ...
    delete phrases;
    /*unsigned int* bm = new unsigned int[(parsing_len+31)/32];
    for(unsigned int i=0;i<(parsing_len+31)/32;i++){
        bm[i] = 0;
    }
    bitset(bm,0);
    ...
    //unsigned int* length = lz77index::utils::readArray(filename+".alph.len",&parsing_len,4);
    patricia* pat_tree = new patricia(text,tlen);
    //pat_tree->add(0);//first phrase starts at position 0 (we could drop this phrase because no one matches it in the reverse)
    pat_tree->add(length[0]);//second phrase starts at position length[0]=1
    //std::cout<<"Pat Tree add:"<<length[0]<<std::endl;
    for(unsigned int i=1;i<parsing_len-1;i++){
        length[i] = length[i] + length[i-1];
        pat_tree->add(length[i]);
        //std::cout<<"Pat Tree add:"<<length[i]<<std::endl;
    }
    ...
    //recompute the skips to use less memory
    /*unsigned int* temp_skip = utils::compressArray(dfuds_skips,nodes,&bits);
    delete [] dfuds_skips;
    if(fwrite(&bits,sizeof(unsigned char),1,lz77index_fp)!=1){
        std::cerr<<"ERROR: while saving the bits of the dfuds skips"<<std::endl;
        return NULL; 
    }
    if(fwrite(temp_skip,sizeof(unsigned int),basics::uint_len(nodes,bits),lz77index_fp) != basics::uint_len(nodes,bits)){
        std::cerr<<"ERROR: while saving the compressed dfuds skips"<<std::endl;
        return NULL; 
    }
    std::cout<<"(S)sst skips: "<<sizeof(unsigned int)*basics::uint_len(nodes,bits)<<std::endl;
    std::cout<<"Skip bits: "<<(unsigned int)bits<<std::endl;
    delete [] temp_skip;*/
    ...
    //std::cout<<"Added Rev trie: "<<0<<" "<<1<<std::endl;
    for(unsigned int i=1;i<parsing_len-1;i++){//we dont want the last phrase
        //std::cout<<"A: "<<length[i]-1<<" "<<length[i]-length[i-1]<<std::endl;
        rev_pat_tree->addReverse(length[i]-1,length[i]-length[i-1]);
        //std::cout<<"Added Rev trie: "<<length[i]-1<<" "<<length[i]-length[i-1]<<std::endl;
    }
    //rev_pat_tree->print();
    delete [] text;
    delete [] length;
    ...
    delete rev_trie;
    //recompute the skips to use less memory    
    /*temp_skip = utils::compressArray(dfuds_skips_rev,nodes_rev,&bits);
    std::cout<<"Skip bits rev: "<<(unsigned int)bits<<std::endl;
    delete [] dfuds_skips_rev;
    if(fwrite(&bits,sizeof(unsigned char),1,lz77index_fp)!=1){
        std::cerr<<"ERROR: while saving the bits of the dfuds skips rev"<<std::endl;
        return NULL; 
    }
    if(fwrite(temp_skip,sizeof(unsigned int),basics::uint_len(nodes_rev,bits),lz77index_fp) != basics::uint_len(nodes_rev,bits)){
        std::cerr<<"ERROR: while saving the compressed dfuds skips rev"<<std::endl;
        return NULL;
    }
    std::cout<<"(S)rev_trie skips: "<<sizeof(unsigned int)*basics::uint_len(nodes_rev,bits)<<std::endl;
    delete [] temp_skip;*/
    ...
    delete wt_depth;
    /*for(unsigned int i=0;i<len;i++){
        std::cout<<aux_phrases[i].id << " " <<aux_phrases[i].start_pos << " " << aux_phrases[i].length<<std::endl;
    }*/
    delete [] depth;
    unsigned int* permutation_array = utils_index::computePermutationArray(aux_phrases,len);
    basics::static_bitsequence_builder* bmb = new basics::static_bitsequence_builder_brw32(20);
    basics::static_permutation* perm = new basics::static_permutation_mrrr(permutation_array,len,32,bmb);//probar que valor funciona mejor
    delete bmb;
    ...
    //delete [] permutation_array; deleted by perm
    delete perm;
    unsigned int* bitmap_sources = utils_index::computeSourcesBitmap(aux_phrases,len);
    phrases = new DeltaCodes(bitmap_sources,len,16);//sampling=8
    /*std::cout<<"Sources:"<<std::endl;    
    for(unsigned int i=0;i<len;i++){
        std::cout<<bitmap_sources[i]<<" ";
    }
    std::cout<<std::endl;*/
    //delete [] length;
    if(phrases->save(lz77index_fp)!=0){
    ...
    //lz77index_fp = fopen((sfile+".lzindex").c_str(),"r");
...
static_selfindex_lz77* static_selfindex_lz77::load(FILE * fp){
    ...
    std::cout<<"parsing_len: "<<ret->parsing_len<<std::endl;
    ...
    /*if(fread(&(ret->sst_skips_bits),sizeof(unsigned char),1,fp)!=1){
        std::cerr<<"ERROR: load: sst skips bits "<<std::endl;
        delete ret; return NULL;
    }
    ret->sst_skips = new unsigned int[basics::uint_len(ret->sst->nodes(),ret->sst_skips_bits)];
    if(ret->sst_skips == NULL){
        std::cerr<<"ERROR: load: allocating sst skips "<<std::endl;
        delete ret; return NULL;
    }
    if(fread(ret->sst_skips,sizeof(unsigned int),basics::uint_len(ret->sst->nodes(),ret->sst_skips_bits),fp) != basics::uint_len(ret->sst->nodes(),ret->sst_skips_bits)){
        std::cerr<<"ERROR: load: sst skips "<<std::endl;
        delete ret; return NULL;
    }*/
    ...
    /*if(fread(&(ret->rev_trie_skips_bits),sizeof(unsigned char),1,fp)!=1){
        std::cerr<<"ERROR: load: rev trie skips bits "<<std::endl;
        delete ret; return NULL; 
    }
    ret->rev_trie_skips = new unsigned int[basics::uint_len(ret->rev_trie->nodes(),ret->rev_trie_skips_bits)];
    if(ret->rev_trie_skips == NULL){
        std::cerr<<"ERROR: load: allocating rev trie skips "<<std::endl;
        delete ret; return NULL;
    }
    if(fread(ret->rev_trie_skips,sizeof(unsigned int),basics::uint_len(ret->rev_trie->nodes(),ret->rev_trie_skips_bits),fp) != basics::uint_len(ret->rev_trie->nodes(),ret->rev_trie_skips_bits)){
        std::cerr<<"ERROR: load: rev trie skips "<<std::endl;
        delete ret; return NULL;
    }*/
...
/*std::vector<unsigned int>* static_selfindex_lz77::_locate(unsigned char* pattern, unsigned int plen, unsigned int* count){
    *count=0;
    std::vector<unsigned int>* occ_pos = new std::vector<unsigned int>();
    ...
    if(Lr>=Ll){
        ...
        if(Rr>=Rl){
            ...
            if(range_res->size()>0){
                ...
                /*unsigned int phrase_end = phrases->select(basics::get_field(rev_ids,rev_ids_bits,(*range_res)[0]-1)+1)+1;

                if(phrase_end<plenL){
                    //std::cout<<"Exiting:"<<plenL<<" "<<plenR<<std::endl;
                    delete range_res;
                    return;
                }*/
...
void static_selfindex_lz77::_searchTreeSst(unsigned char* pattern, unsigned int plen, unsigned int* L, unsigned int* R){
    ...
    while(pos<plen && !sst->inspect(node)){//sst->isleaf(node)
        ...
        //pos += basics::get_field(sst_skips,sst_skips_bits,s_pos);
...
void static_selfindex_lz77::_searchTreeRev(unsigned char* pattern, unsigned int plen, unsigned int* L, unsigned int* R){
    ...
    while(pos<plen && !rev_trie->inspect(node)){//rev_trie->isleaf(node)
        ...
        //pos += basics::get_field(rev_trie_skips,rev_trie_skips_bits,s_pos);
...
void static_selfindex_lz77::_computeSecondaryOcc(unsigned int pos, unsigned int plen, std::vector<unsigned int>* occ_pos, unsigned int max_occs){
    ...
    while(1){
        ...
        //while(source_start+_source_length(perm->rev_pi(source_id))-1 >= pos+plen){
...
void static_selfindex_lz77::_binarySearchRev(unsigned char* pattern, unsigned int plen, unsigned int* left, unsigned int* right){
    ...
    while(lower<=upper){
        ...
        //std::cout<<"extracting "<<phrase_end<<" "<<m<<" "<<basics::get_field(rev_ids,rev_ids_bits,m)<<" "<<phrase_end-l+1<<" "<<phrase_end<<std::endl;
...
void static_selfindex_lz77::_binarySearchSst(unsigned char* pattern, unsigned int plen, unsigned int* left, unsigned int* right){
    ...
    while(lower<=upper){
        ...
        //std::cout<<"extracting "<<phrase_start<<" "<<m<<" "<<lower<<" "<<upper<<" "<<basics::get_field(sst_ids,sst_ids_bits,m)+1<<" "<<phrase_start+plen-1<<" "<<std::endl;
