...
int inPatterns(int a, int* patterns, int l, int t, unsigned char* text){
    unsigned int i;
    for(i=0;i<t;i++){
        int j;
        for(j=0;j<l;j++){
            if(text[a+j]!=text[patterns[i]+j])break;
        }
        if(j==l)return 1;
    }
    return 0;
...
main (int argc, char **argv)
{
    ...
    int* patterns = (int*)malloc(sizeof(int)*J);
	for (t = 0; t < J; t++)
	{
		int j, l;
        int cnt=0;
        ...
        }while(cnt<100 && inPatterns(j,patterns,m,t,buff));
        patterns[t]=j;
		for (l = 0; l < m; l++)
